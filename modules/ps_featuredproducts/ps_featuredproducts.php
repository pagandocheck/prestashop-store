...
class Ps_FeaturedProducts extends Module implements WidgetInterface
{
    ...
    public function hooktabHomeContent(array $configuration = []){
            ...
            $this->smarty->assign($variables);
        return $this->fetch('module:ps_featuredproducts/views/templates/hook/tabcontent.tpl', $this->getCacheId('ps_featuredproducts'));
    }
    public function hookTabHome($params)
	{
		$products = $this->getProducts();
        if (!empty($products)) {
		  return $this->display(__FILE__, 'tab.tpl', $this->getCacheId('ps_featuredproducts'));
        }
        return false;
	}
